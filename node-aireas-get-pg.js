...
//			_attributes = _attributes + " , ST_AsGeoJSON(ST_Transform(airbox.geom28992, 4326)) geom4326 "; 
	...
	getGridApriSensorInfo: function (param, req_query, callback) {
		...
		var queryWhere = " where area.area_code = '" + req_query.areaCode + "' and area.area_code = gc.area_code and gc.gid = gcu.gridcell_gid ";
			...
			queryWhere += " and gcu.avg_datetime = (select max(avg_datetime) from as_gridcell gc2, as_gridcell_union gcu2 where 1=1 and gcu2.gridcell_gid = gc2.gid and gc2.area_code = area.areacode " + avgDateTimeMaxConstraintStr + ")";
		var queryGroupBy = ""; 
		var queryOrderBy = ""; 
...
/* Quarter mean values
	SELECT hist_year, round((hist_month+2)/3) Q, sum(hist_count), avg_type, round(avg(avg_avg)) avg
  FROM public.aireas_hist_avg
  where hist_day is null
  and hist_month is not null
  --and airbox = '13.cal'
  group by avg_type, hist_year, Q
  order by avg_type, hist_year, Q
  limit 100
*/	
