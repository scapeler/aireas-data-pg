...
*/
			...
			if (toprecord1 == 'foiCode;eventDate;Datum;Tijdstip;Gebeurtenis;opmerking;lat;lng') {
...
//				}
			...
			for (i=1;i<inputFile.length;i++) {
				...
				var _foiCode, _eventDate, _date, _time, _eventDesc, _remarks, _lat, _lng ;
				_foiCode 			= null;
				_eventDate 			= null;
				_date 				= null;
				_time 				= null;
				...
				if (recordType == 1 ) {
					...
					_eventDate 		= inputRecordCols[1];
					_date 			= inputRecordCols[2];
					_time 			= inputRecordCols[3]; //new Date(inputRecordCols[3]);
					_eventDesc 		= inputRecordCols[4];
					_remarks 		= inputRecordCols[5];
					_lat 			= _parseFloat(inputRecordCols[6]);
					_lng 			= _parseFloat(inputRecordCols[7]);
				};	


				outputRecord = "\nINSERT INTO aera_import_event ( foi_code, event_date, " + 
						" event_day, event_time, event_desc, event_remarks, lat, lng " +
						" ) VALUES (" +
						"'" + 	_foiCode 		+ "', " +
						"'" + 	_eventDate		+ "', " +			// UTC timestamp
						"'" + 	_date 			+ "', " +
						"'" + 	_time 			+ "', " +
						"'" + 	_eventDesc 		+ "', " +
						"'" + 	_remarks 		+ "', " +
			...
			console.log('Write: ' + outputFilePath+_foiCode+'_event_test.sql');	
//		writeFile(outputFile, outputFilePath+'test.sql');
			fs.writeFileSync(outputFilePath+_foiCode+'_event_test.sql', outputFile);
